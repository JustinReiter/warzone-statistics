...
router.get('/', function(req, res, next) {
    db.any("SELECT * FROM ladders, templates, daily_standings WHERE ladders.lid=templates.lid AND templates.lid=daily_standings.lid;")
    .then((ladders) => {
...
router.get('/:ladderId', function(req, res, next) {
    if (req.params.ladderId && !isNaN(req.params.ladderId))  {
        db.any("SELECT * FROM ladders, templates, daily_standings WHERE ladders.lid=$1 AND ladders.lid=templates.lid AND templates.lid=daily_standings.lid;",
            [req.params.ladderId])
        .then((ladder) => {
                if (!ladder) {
